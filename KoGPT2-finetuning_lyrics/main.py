...
def main(epoch, save_path, load_path, samples, data_file_path, batch_size):
	...
	model_path='C:/Users/hhz20/산공과제/lyric___/NLP/KoGPT2-finetuning_lyrics/kogpt2/model/checkpoint/pytorch_kogpt2_676e9bcfa7.params'
	...
	for epoch in range(epoch):
		for data in data_loader:
			...
			# generator 진행
			if (count > 0 and count % 4000 == 0) or (len(data) < batch_size): # 1000
				...
				if count > 500000:
					...
					now = max(now)
					f = open(samples + str(now + 1), 'w', encoding="cp949")
					f.write(sent)
					f.close()
			#########################################
			count += 1

			if (count > 0 and count % 4000 == 0) or (len(data) < batch_size):  #10000
				# 모델 저장
				try:
					torch.save({
						'epoch': epoch,
